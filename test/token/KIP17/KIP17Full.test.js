...
const KIP17FullMock = artifacts.require('KIP17FullMock.sol');

contract('KIP17Full', function ([
  creator,
  ...accounts
]) {
  ...
  beforeEach(async function () {
    this.token = await KIP17FullMock.new(name, symbol, { from: creator });
  });

  describe('like a full KIP17', function () {
    beforeEach(async function () {
    ...
    describe('burn', function () {
      ...
      it('burns all tokens', async function () {
        ...
        await shouldFail.reverting.withMessage(
          this.token.tokenByIndex(0), 'KIP17Enumerable: global index out of bounds'
        );
      });
    });
    ...
    describe('metadata', function () {
      ...
      it('reverts when setting metadata for non existent token id', async function () {
        await shouldFail.reverting.withMessage(
          this.token.setTokenURI(nonExistentTokenId, sampleUri), 'KIP17Metadata: URI set of nonexistent token'
        );
      });
      ...
      it('reverts when querying metadata for non existent token id', async function () {
        await shouldFail.reverting.withMessage(
          this.token.tokenURI(nonExistentTokenId), 'KIP17Metadata: URI query for nonexistent token'
        );
      });
    });
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(owner, 2), 'KIP17Enumerable: owner index out of bounds'
          );
        });
      });
      ...
      describe('when the given address does not own any token', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(another, 0), 'KIP17Enumerable: owner index out of bounds'
          );
        });
      });
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns empty collection for original owner', async function () {
          ...
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(owner, 0), 'KIP17Enumerable: owner index out of bounds'
          );
        });
      });
    });
    ...
    describe('tokenByIndex', function () {
      ...
      it('should revert if index is greater than supply', async function () {
        await shouldFail.reverting.withMessage(
          this.token.tokenByIndex(2), 'KIP17Enumerable: global index out of bounds'
        );
      });
