...
const KIP17ReceiverMock = artifacts.require('KIP17ReceiverMock.sol');
const ERC721ReceiverMock = artifacts.require('ERC721ReceiverMock.sol');

function shouldBehaveLikeKIP17 (
  creator,
  minter,
  ...
  const RECEIVER_MAGIC_VALUE = '0x6745782b';

  describe('like an KIP17', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await shouldFail.reverting.withMessage(
            this.token.balanceOf(ZERO_ADDRESS), 'KIP17: balance query for the zero address'
          );
        });
      });
    });
        ...
        context('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await shouldFail.reverting.withMessage(
              ...
              'KIP17: transfer to the zero address'
            );
          });
        });
      };
      ...
      describe('via safeTransferFrom', function () {
        ...
        const shouldTransferSafely = function (transferFun, data) {
          ...
          describe('to a valid receiver contract', function () {
            beforeEach(async function () {
              this.receiver = await KIP17ReceiverMock.new(RECEIVER_MAGIC_VALUE);
              this.ERC721receiver = await ERC721ReceiverMock.new(ERC721_RECEIVER_MAGIC_VALUE);
            ...
            it('should call onERC721Received', async function () {
              const receipt = await transferFun.call(this, owner, this.ERC721receiver.address, tokenId, { from: owner });
            ...
            it('should call onKIP17Received', async function () {
              const receipt = await transferFun.call(this, owner, this.receiver.address, tokenId, { from: owner });

              await expectEvent.inTransaction(receipt.tx, KIP17ReceiverMock, 'Received', {
                operator: owner,
                from: owner,
                tokenId: tokenId,
                data: data,
            ...
            it('should call onERC721Received from approved', async function () {
              const receipt = await transferFun.call(this, owner, this.ERC721receiver.address, tokenId, { from: approved });
            ...
            it('should call onKIP17Received from approved', async function () {
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await KIP17ReceiverMock.new('0x42');
            await shouldFail.reverting.withMessage(
    ...
    describe('approve', function () {
      ...
      context('when the address that receives the approval is the owner', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.approve(owner, tokenId, { from: owner }), 'KIP17: approval to current owner'
          );
        });
      });
      ...
      context('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(this.token.approve(approved, tokenId, { from: other }),
            'KIP17: approve caller is not owner nor approved');
        });
      });
