...
const KIP37ReceiverMock = artifacts.require('KIP37ReceiverMock');
const ERC1155ReceiverMock = artifacts.require('ERC1155ReceiverMock');

function shouldBehaveLikeKIP37 ([minter, firstTokenHolder, secondTokenHolder, multiTokenHolder, recipient, proxy]) {
  ...
  const thirdTokenAmount1 = new BN(2000);
  const thirdTokenAmount2 = new BN(2200);
  ...
  describe('like an KIP37', function () {
      ...
      context('when accounts own some tokens', function () {
        ...
        it('returns the amount of tokens owned by the given addresses', async function () {
          ...
          expect(await this.token.balanceOf(
            firstTokenHolder,
            thirdTokenId,
          )).to.be.bignumber.equal(thirdTokenAmount1);

          expect(await this.token.balanceOf(
            secondTokenHolder,
            thirdTokenId,
          )).to.be.bignumber.equal(thirdTokenAmount2);

        });

        it('returns the total supply of the given token', async function() {
          expect(await this.token.totalSupply(thirdTokenId)).to.be.bignumber.equal(thirdTokenAmount1.add(thirdTokenAmount2))
        })
      ...
      context('when sending to a valid receiver', function () {
        ...
        context('without data', function () {
          ...
          it('calls onKIP37BatchReceived', async function () {
            await expectEvent.inTransaction(this.transferReceipt.tx, KIP37ReceiverMock, 'BatchReceived', {
              operator: multiTokenHolder,
              from: multiTokenHolder,
              // ids: [firstTokenId, secondTokenId],
              // values: [firstAmount, secondAmount],
    ...
    shouldSupportInterfaces(['KIP13', 'KIP37']);
  });
}
