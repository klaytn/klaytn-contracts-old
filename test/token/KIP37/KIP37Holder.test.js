...
const KIP37Holder = artifacts.require('KIP37Holder');
...
contract('KIP37Holder', function (accounts) {
  ...
  beforeEach(async function () {
    this.multiToken = await KIP37Mock.new(uri, { from: creator });
    this.holder = await KIP37Holder.new();
    this.token = this.holder;
    await this.multiToken.mintBatch(creator, multiTokenIds, multiTokenAmounts, '0x', { from: creator });
  });

  shouldSupportInterfaces(['KIP13', 'KIP37Receiver']);

  it('receives KIP37 tokens from a single ID', async function () {
  ...
  it('receives KIP37 tokens from a multiple IDs', async function () {
